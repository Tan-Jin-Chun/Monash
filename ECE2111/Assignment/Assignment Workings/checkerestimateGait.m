...
%% Section 3.3
function [STl, STr, SWl, SWr, Sl, Sr] = checkerestimateGait(VGRF)
...
% Sr vector of the stride times for the right foot
    ...
    % and it is not the left stance phase
        if ((~L_Stance_Phase) && left_foot(i) >= threshold_val)
    ...
    % left swing phase
        elseif ((~L_Swing_Phase) && left_foot(i) < threshold_val)
    ...
    % and it is not the right stance phase
        if ((~R_Stance_Phase) && right_foot(i) >= threshold_val)
    ...
    % right swing phase
        elseif ((~R_Swing_Phase) && right_foot(i) < threshold_val)
