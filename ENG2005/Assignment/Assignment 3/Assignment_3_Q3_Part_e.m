...
% Parameters
N = 6;
m = 100*[9; 4; 4; 7; 1; 0.22]; % part (a) and later part (e) m(5) obtain through guessing
k = 10000*ones(1,N);           % part (a) and later part (e)
c = [200*ones(1,N-1), 20];     % part (a) and later part (e)
...
for i = 2:Nt
    grad_1 = RHS(x(:,i-1), t(i-1));
    newx = x(:,i-1) + dt*grad_1;
    grad_2 = RHS(newx, t(i));
    x(:,i) = x(:,i-1) + dt/2*(grad_1+grad_2);
end
...
xlabel('t')
ylabel('Displacement')
newcolors = {'#F00','#F80','#FF0','#0B0','#00F','#800080','#FF0090','#FF0090'};
colororder(newcolors)
legend('Floor 1','Floor 2','Floor 3','Floor 4','Floor 5','Floor 6','Threshold','Threshold');
