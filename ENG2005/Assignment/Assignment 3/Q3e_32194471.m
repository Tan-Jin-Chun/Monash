...
N = 6; % Add a new floor (for pendulum)
m = 100*[9,4,4,7,1,70]; % part (a) and later part (e) % Play around with the value of m6 (10000 def val)
k = 10000*ones(1,N); % part (a) and later part (e)
c = [200*ones(1,N-1),20];   % part (a) and later part (e) % c6 is 20 not 200
...
x = zeros(2*N,Nt); %here we store the solution at each timestep in separate (Nt) columns
 ...
 for i = 2:Nt
     ...
     x(:,i) = x(:,i-1) + dt/2 * (grad_A + grad_B); % part (b) and then part (d) (two different codes, you will probably use more than one line for part (d))
 ...
 newcolors = {'#F00','#F80','#FF0','#0B0','#00F','#FF00FF'};
 colororder(newcolors)

 % Added Code (Added Floor 6 -> Pendulum)
 yline(0.04,'k--','Threshold', 'LineWidth',2);
 yline(-0.04,'k--','Threshold','LineWidth',2);
 legend('Floor 1','Floor 2','Floor 3','Floor 4','Floor 5','Floor 6','Positive Threshold','Negative Threshold');
