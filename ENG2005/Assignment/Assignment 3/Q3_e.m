...
%parameters
N = 6;
m = 100 * [9, 4, 4, 7, 1, 1000];     % part (e)
k = 10000 * ones(1,N);              % part (e)
c = [200*ones(1,N-1), 20];          % part (e)
...
x = zeros(2*N,Nt); %here we store the solution at each timestep in separate (Nt) columns
 ...
 %for accuracy.
 for i = 2:Nt
 gradA = RHS(x(:,i-1),t(i-1)); % part (b) and then part (d) (two different codes, you will probably use more than one line for part (d))
 xPredict = x(:,i-1) + dt*gradA;
 gradB = RHS(xPredict,t(i));
 x(:,i) = x(:,i-1) + dt * (gradA + gradB)/2;
 end
