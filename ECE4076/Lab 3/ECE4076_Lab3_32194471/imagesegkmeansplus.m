function [segmented_img, final_centroids, total_iter] = imagesegkmeansplus(img, k, max_iter, ...
 ...
 % random seed  
    ...
    distance2centroid = ones(length(flatten_img),k) * 450;
    init_centroid = ones(k,3)*-999; %invalid value
    r = randi([1, length(flatten_img)]);
    first_centroid = flatten_img(r, :);
    init_centroid(1, :) = first_centroid;
    % intialization through weighted sampling
    % higher the probability, higher the chance to get sampled as
    % means/centroids
    for c = 1:k-1
        selected_centroid = init_centroid(c,:);
        temp_norm = (flatten_img - repmat(selected_centroid, width*height, 1)).^2;
        temp_ed = sqrt(sum(temp_norm, 2));
        distance2centroid(:, c) = temp_ed;
        [min_dist, min_idx] = min(distance2centroid, [], 2);
        Dx = min_dist;
        Dx2 = min_dist;
        probability = Dx2./sum(Dx2);
        r = randsample(width*height, 1, true, probability);
        init_centroid(c+1,:) = flatten_img(r, :);
    end
