...
% Set Filter width to 1,0.6,0.4
FilterWidth=0.6;                  % Determines LPF cutoff (normalised frequency)
...
% Set the bitpattern to 1,2
BitPattern = 1;     % enter values 1 or 2 or 3
if BitPattern==1
...
% I have changed the displaybits variable to divide by 3 or just remove the
% divide by 2 operation
elseif BitPattern==3
    BitPattern=zeros(1, NumInputBits);
    BitPattern(round(DisplayBits/3))=1;             % Single rectangular pulse
...
% INsert of o
plot(((frequencies)/(2*pi*Norm_freq)),Frequency_response_in_dB1,'-o') %Plot the normalised frequency response
